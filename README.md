# Тестовое задание “Prefix Search Ranking” - Реализация на OpenSearch
...
*   **Python-скрипты:**
    ...
    *   `search_engine.py`: Выполняет запросы к OpenSearch для каждого префикса в `data/prefix_queries.csv` и сохраняет результаты в `reports/elasticsearch_evaluation_results.csv`.
...
3.  **Загрузка данных и запуск оценки (Способ 1 - Прямое выполнение Python):**
    ...
    *   Запустите скрипт поиска и оценки:
        ...
        ```
    *   Результаты будут сохранены в `reports/elasticsearch_evaluation_results.csv`.
...
5.  **Оценка покрытия:**
    *   После запуска `search_engine.py` можно рассчитать покрытие с помощью предоставленного скрипта:
        ...
        ```
        Этот скрипт читает `reports/elasticsearch_evaluation_results.csv` и выводит процент покрытия для `open` запросов.

## Результаты

*   **Покрытие:** Решение достигло **76.67%** покрытия релевантных `open` запросов, превысив порог 70%.
*   **Выходной файл:** Детальные результаты хранятся в `reports/elasticsearch_evaluation_results.csv`.
...
*   `reports/`: Директория, в которую сохраняется выходной отчёт (`elasticsearch_evaluation_results.csv`).
